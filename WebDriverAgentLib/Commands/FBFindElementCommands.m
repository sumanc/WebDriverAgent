...
{
  FBSession *session = request.session;
  NSArray *elements = [self.class elementsUsing:(NSString *)request.arguments[@"using"] withValue:(NSString *)request.arguments[@"value"] under:session.application
...
+ (id<FBResponsePayload>)handleFindSubElements:(FBRouteRequest *)request
{
  FBElementCache *elementCache = request.session.elementCache;
  XCUIElement *element = [elementCache elementForUUID:request.parameters[@"uuid"]];
  NSArray *foundElements = [self.class elementsUsing:(NSString *)request.arguments[@"using"] withValue:(NSString *)request.arguments[@"value"] under:element
                         shouldReturnAfterFirstMatch:NO];
